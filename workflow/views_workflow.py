...
def usuario_tiene_acceso_completo_solicitud(usuario, solicitud):
    """
    Verifica si un usuario tiene acceso completo a una solicitud considerando:
    1. Propietario de la solicitud
    2. Usuario asignado a la solicitud
    3. Supervisión de grupos
    4. Permisos por rol existentes
    
    Esta función extiende la funcionalidad existente sin reemplazarla.
    """
    print(f"🔍 DEBUG ACCESO COMPLETO: Verificando acceso para usuario {usuario.username} en solicitud {solicitud.id}")
    
    # 1. Verificar si es propietario o asignado (MANTENER LÓGICA EXISTENTE)
    if (solicitud.creada_por == usuario or 
        solicitud.asignada_a == usuario or 
        usuario.is_superuser or 
        getattr(usuario, "is_staff", False)):
        print(f"✅ DEBUG: Usuario {usuario.username} tiene acceso directo a la solicitud")
        return True

    # 2. Verificar supervisión de grupos (NUEVA FUNCIONALIDAD)
    try:
        from pacifico.utils_grupos import obtener_grupos_supervisados_por_usuario
        
        # Obtener grupos que el usuario supervisa
        grupos_supervisados = obtener_grupos_supervisados_por_usuario(usuario)
        
        if grupos_supervisados.exists():
            print(f"🔍 DEBUG: Usuario {usuario.username} supervisa {grupos_supervisados.count()} grupos")
            
            # Verificar si la solicitud pertenece a un grupo supervisado
            grupos_supervisados_ids = [gp.group.id for gp in grupos_supervisados]
            
            # Verificar si el propietario de la solicitud está en un grupo supervisado
            if solicitud.creada_por and solicitud.creada_por.groups.filter(id__in=grupos_supervisados_ids).exists():
                print(f"✅ DEBUG: Usuario {usuario.username} es supervisor del grupo del propietario de la solicitud")
                return True
            
            # Verificar si la solicitud está en un pipeline con grupos supervisados
            if solicitud.pipeline and solicitud.pipeline.grupos.filter(id__in=grupos_supervisados_ids).exists():
                print(f"✅ DEBUG: Usuario {usuario.username} es supervisor de un grupo del pipeline de la solicitud")
                return True
    ...
    print(f"❌ DEBUG: Usuario {usuario.username} NO tiene acceso completo a la solicitud")
    return False


def usuario_puede_ver_solicitud_por_supervision(usuario, solicitud):
    """
    Verifica específicamente si un usuario puede ver una solicitud por supervisión de grupos.
    Esta función es más específica y se enfoca solo en la supervisión.
    """
    try:
        from pacifico.utils_grupos import obtener_grupos_supervisados_por_usuario
...
def obtener_etapas_con_bandeja_para_usuario(usuario):
    """
    Obtiene etapas con bandeja grupal considerando supervisión.
    MANTIENE toda la funcionalidad existente y agrega la supervisión.
    """
    print(f"🔍 DEBUG ETAPAS: Obteniendo etapas con bandeja para usuario {usuario.username}")
    
    etapas_con_bandeja = []
    
    # Para superusers y staff (MANTENER LÓGICA EXISTENTE)
    if usuario.is_superuser or usuario.is_staff:
        ...
        print(f"✅ DEBUG: Usuario {usuario.username} es superuser/staff, acceso completo a {len(etapas_con_bandeja)} etapas")
        ...
        if user_role in ['Analista', 'Back Office']:
            # === LÓGICA EXISTENTE + SUPERVISIÓN ===
            
            # 1. Obtener etapas por permisos directos (MANTENER)
            etapas_directas = Etapa.objects.filter(
                es_bandeja_grupal=True,
                permisos_bandeja__usuario=usuario,
                permisos_bandeja__puede_ver=True
            ).exclude(nombre__iexact="Comité de Crédito").select_related('pipeline').distinct()
            
            # 2. Obtener etapas por supervisión (NUEVO)
            etapas_supervision = []
            try:
                from pacifico.utils_grupos import obtener_grupos_supervisados_por_usuario
                grupos_supervisados = obtener_grupos_supervisados_por_usuario(usuario)
                ...
                if grupos_supervisados.exists():
                    print(f"🔍 DEBUG: Usuario {usuario.username} (rol: {user_role}) supervisa {grupos_supervisados.count()} grupos")
                    # Si es supervisor, puede ver todas las etapas de bandeja grupal
                    etapas_supervision = Etapa.objects.filter(
                        es_bandeja_grupal=True
                    ).exclude(nombre__iexact="Comité de Crédito").select_related('pipeline').distinct()
                    print(f"✅ DEBUG: Usuario {usuario.username} tiene acceso por supervisión a {len(etapas_supervision)} etapas")
            ...
            etapas_con_bandeja = etapas_unicas
            
            print(f"✅ DEBUG: Usuario {usuario.username} (rol: {user_role}) tiene acceso a {len(etapas_con_bandeja)} etapas totales")
            
        else:
            # === LÓGICA EXISTENTE + SUPERVISIÓN ===
            
            # 1. Obtener etapas por grupos del usuario (MANTENER)
            grupos_usuario = usuario.groups.all()
            etapas_grupos = Etapa.objects.filter(
                es_bandeja_grupal=True,
            ...
            # 3. Combinar y eliminar duplicados
            todas_etapas = list(etapas_grupos) + list(etapas_supervision)
            seen_ids = set()
            etapas_unicas = []
            for etapa in todas_etapas:
                if etapa.id not in seen_ids:
    ...
    return etapas_con_bandeja


def obtener_solicitudes_por_supervision(usuario):
    """
    Obtiene solicitudes que un usuario puede ver por supervisión de grupos.
    Esta función es útil para filtrar solicitudes en vistas y dashboards.
    """
    try:
        from pacifico.utils_grupos import obtener_grupos_supervisados_por_usuario
        ...
        if grupos_supervisados.exists():
            ...
            # Obtener solicitudes de grupos supervisados
            # Usar solo los campos básicos para evitar problemas con union()
            solicitudes_supervision = Solicitud.objects.filter(
                creada_por__groups__id__in=grupos_supervisados_ids
            ).exclude(creada_por=usuario)
            
            return solicitudes_supervision
...
def obtener_cotizaciones_por_supervision(usuario):
    """
    Obtiene cotizaciones que un usuario puede ver por supervisión de grupos.
    Esta función es útil para filtrar cotizaciones en vistas y dashboards.
    """
    try:
        from pacifico.utils_grupos import obtener_grupos_supervisados_por_usuario
        ...
        if grupos_supervisados.exists():
            ...
            # Obtener cotizaciones de grupos supervisados
            # Usar solo los campos básicos para evitar problemas con union()
            cotizaciones_supervision = Cotizacion.objects.filter(
                added_by__groups__id__in=grupos_supervisados_ids
            ).exclude(added_by=usuario)
            
            return cotizaciones_supervision
...
def api_buscar_cotizaciones_drawer(request):
    ...
    if request.method == 'GET':
        ...
        try:
            ...
            print(f"❌ ERROR en api_buscar_cotizaciones_drawer: {e}")
            import traceback
            traceback.print_exc()
...
def vista_mixta_bandejas(request):
    ...
    etapas_con_bandeja = obtener_etapas_con_bandeja_para_usuario(request.user)
...
def api_tomar_solicitud(request, solicitud_id):
    ...
    try:
        ...
        # === SISTEMA DE PERMISOS INTEGRADO CON SUPERVISIÓN ===
        ...
        if request.user.is_staff:
            ...
            # 1. Verificar permisos por grupos (MANTENER LÓGICA EXISTENTE)
            grupos_usuario = request.user.groups.all()
            ...
            # 3. Si no tiene permisos directos, verificar por supervisión (NUEVA FUNCIONALIDAD)
            if not tiene_permiso:
                try:
                    from pacifico.utils_grupos import obtener_grupos_supervisados_por_usuario
                    grupos_supervisados = obtener_grupos_supervisados_por_usuario(request.user)
                    ...
                    if grupos_supervisados.exists():
                        ...
                        # Verificar permisos de bandeja por supervisión
                        tiene_permiso = PermisoBandeja.objects.filter(
                            etapa=solicitud.etapa_actual,
                            grupo__id__in=grupos_supervisados_ids,
                            puede_tomar=True
                        ).exists()
                        
                        # Verificar permisos de etapa por supervisión
                        if not tiene_permiso:
                            tiene_permiso = PermisoEtapa.objects.filter(
                                etapa=solicitud.etapa_actual,
                                grupo__id__in=grupos_supervisados_ids,
                        ...
                        if tiene_permiso:
                            print(f"✅ DEBUG: Usuario {request.user.username} tiene permisos por supervisión")
...
def api_devolver_solicitud(request, solicitud_id):
    ...
    try:
        ...
        # === SISTEMA DE PERMISOS SUPER STAFF + SUPERVISIÓN ===
        ...
        if request.user.is_staff:
            ...
            # Verificar si puede devolver por supervisión (NUEVA FUNCIONALIDAD)
            ...
            try:
                ...
                if grupos_supervisados.exists():
                    ...
                    # Verificar si el propietario de la solicitud está en un grupo supervisado
                    if solicitud.creada_por and solicitud.creada_por.groups.filter(id__in=grupos_supervisados_ids).exists():
                        print(f"✅ DEBUG: Usuario {request.user.username} puede devolver solicitud por supervisión")
                        puede_devolver = True
                    
                    # Verificar si la solicitud está en un pipeline con grupos supervisados
                    elif solicitud.pipeline and solicitud.pipeline.grupos.filter(id__in=grupos_supervisados_ids).exists():
                        print(f"✅ DEBUG: Usuario {request.user.username} puede devolver solicitud por supervisión de pipeline")
                        puede_devolver = True
