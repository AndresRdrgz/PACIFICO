...
def generate_report_excel(request, numero_cotizacion):
    ...
    return generate_report(request, numero_cotizacion)
...
def _create_populated_excel(cotizacion, numero_cotizacion):
    ...
    # Load the workbook and populate it (simplified version for now)
    workbook = load_workbook(excel_path)
    ...
    if "COTIZADOR PREST. AUTO" in workbook.sheetnames:
        ...
        raise ValueError("Sheet 'COTIZADOR PREST. AUTO' not found.")
    
    # Populate the main sheet (using same logic as original function)
    sheet['D6'] = resultado['oficial']
    sheet['C10'] = resultado['nombreCliente']
    sheet['G10'] = resultado['cedulaCliente']
    sheet['H10'] = resultado['tipoDocumento']
...
def _convert_excel_to_pdf(excel_path, client_name, numero_cotizacion):
    ...
    # Title
    title = Paragraph(f"Consulta de Préstamo Automotriz - {numero_cotizacion}", styles['Title'])
    content.append(title)
    content.append(Spacer(1, 0.2*inch))
    
    # Client info
    client_info = Paragraph(f"Cliente: {client_name}", styles['Normal'])
    ...
    if "COTIZADOR PREST. AUTO" in workbook.sheetnames:
        ...
        # Convert key information to table format
        key_data = [
            ['Campo', 'Valor'],
            ['Cliente', str(sheet['C10'].value or '')],
            ['Cédula', str(sheet['G10'].value or '')],
            ['Edad', str(sheet['J10'].value or '')],
            ['Valor del Auto', str(sheet['C14'].value or '')],
            ['Abono', str(sheet['e15'].value or '')],
            ['Plazo', str(sheet['F14'].value or '') + ' meses'],
            ['Letra Mensual', str(sheet['E41'].value or '')],
            ['Total a Pagar', str(sheet['E46'].value or '')],
        ]
        
        table = Table(key_data)
